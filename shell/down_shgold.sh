...
newfile="shgold_`date +'%Y%m%d%H%M%S'`.json"
fileName="shgold_`date +'%Y%m%d'`.json"
...
#下载文件
wget -q -O ${fileName} http://web.juhe.cn:8080/finance/gold/shgold?key=d079671d39ae5259cb9dba6fe1b8828c
echo "`date +'%Y%m%d%H%M%S'`  ${fileName} 下载完毕!" >> temp.log

#转换文件
python /home/huang/data/json2del.py 'shgold' `date +%Y%m%d`
